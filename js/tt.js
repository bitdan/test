    ...
    */
    ...
    function setdf(xuh) {
        if (xuh == "0") {
            ...
            var sumje = jQuery("#sumvalue10494").val().replace(/,/gm, '');
            jQuery("#field6961_" + xuh).val(sumje);//金额
            jQuery("#field6961_" + xuh + "span").text("");
            if (sumje < 0) {
                jQuery("#field6955_" + xuh).val("29");//负数时，贷方记账码为29
                jQuery("#field6955_" + xuh + "span").text("29");
                jQuery("#field6961_" + xuh).val(Math.abs(sumje));//负数时，金额取绝对值
                jQuery("#field6961_" + xuh + "span").text("");
                jQuery("#field10218_" + xuh).val("7");//负数时，总账标识为7
