jQuery(document).ready(function () {
    ...
    function setjf(xuh) {
        ...
        var Pickupordernumber = jQuery("#field23865_" + xuh + "span").text(); //提货单号
        if (parseFloat(xuh) < indexnum0) {
            if (lx == "0") {//配送 
                var jzm = "40";
                jQuery("#field18382_" + xuh).val("6601340000"); //借方科目编号
                jQuery("#field18382_" + xuh + "span").text("6601340000");
                jQuery("#field18357_" + xuh).val("营业费用_运费及杂费"); //借方科目描述
                jQuery("#field18357_" + xuh + "span").text("");
                jQuery("#field18401_" + xuh).val(sqr + "_" + "报配送" + "_" + Pickupordernumber); //借方行项目文本
                jQuery("#field18401_" + xuh + "span").text(""); //借方行项目文本
            } else if (lx == "1") {//转储
                var jzm = "25";
                var khbm = jQuery("#field18362_" + xuh + "span").text(); //客户/供应商编码
                var khms = jQuery("#field18363_" + xuh + "span").text(); //客户/供应商名称

                jQuery("#field18382_" + xuh).val(khbm); //借方科目编码
                jQuery("#field18382_" + xuh + "span").text(khbm);
                jQuery("#field18357_" + xuh).val(khms); //借方科目描述
                jQuery("#field18357_" + xuh + "span").text("");

                jQuery("#field18401_" + xuh).val(sqr + "_" + "报转储" + "_" + sapzcd); //借方行项目文本
                jQuery("#field18401_" + xuh + "span").text(""); //借方行项目文本
            } else {//调拨或者其他
                var jzm = "40";
                jQuery("#field18382_" + xuh).val("6601340000"); //借方科目编号
                ...
                jQuery("#field18401_" + xuh).val(sqr + "_" + "报调拨" + "_" + sapjhd); //借方行项目文本
                jQuery("#field18401_" + xuh + "span").text(""); //借方行项目文本
            }
            jQuery("#field18356_" + xuh).val(jzm); //借方记账码
            jQuery("#field18356_" + xuh + "span").text(jzm);

            var freightje = jQuery("#field18396_" + xuh + "span").text().replace(/,/gm, ''); //运费金额
            jQuery("#field18373_" + xuh).val(freightje); //借方金额
            jQuery("#field18373_" + xuh + "span").text("");

            var fymxbmid = jQuery("#field18582_" + xuh).val(); //费用承担部门ID
            if (fymxbmid != "") {
                var fieldname = "hsdm";
                var fieldnamet = "hsmc";
    ...
    function setdf(xuh) {
        ...
        if (xuh == "0") {
            ...
            jQuery("#field18391_" + xuh).val(jzm); //记账码
            jQuery("#field18391_" + xuh + "span").text(jzm);

            if (lx == "0") {
                jQuery("#field18383_" + xuh).val(sqr + "_" + "报配送" + "_" + Pickupordernumber); //贷方行项目文本
            } else if (lx == "1") {
                jQuery("#field18383_" + xuh).val(sqr + "_" + "报转储" + "_" + sapzcd); //贷方行项目文本
            } else {
                jQuery("#field18383_" + xuh).val(sqr + "_" + "报调拨" + "_" + sapjhd); //贷方行项目文本
            }


            var sumje = jQuery("#sumvalue18396").val().replace(/,/gm, '');

            jQuery("#field18384_" + xuh).val(sumje); //贷方金额=运费金额
            jQuery("#field18384_" + xuh + "span").text("");


            jQuery("#field18393_" + xuh).val("6051040000"); //贷方科目编号
            jQuery("#field18393_" + xuh + "span").text("6051040000");
            jQuery("#field18360_" + xuh).val("其他业务收入_运费收入"); //贷方科目描述
            jQuery("#field18360_" + xuh + "span").text("");
        }
    }
