jQuery(document).ready(function () {
    ...
    */
    ...
    function setbm() {
        ...
        var bzval = jQuery("#field11469").val();
        var bz = jQuery("#field11469span").text();
        for (var i = 0; i < indexnum; i++) {
            jQuery("#field11509_" + i).val(bzval);
            jQuery("#field11509_" + i + "span").text(bz);
        }
    }

    function setbmByadd() {
        var indexnum = document.getElementById("indexnum0").value * 1.0 - 1;
        var bzval = jQuery("#field11469").val();
        var bz = jQuery("#field11469span").text();
        jQuery("#field11509_" + indexnum).val(bzval);
        jQuery("#field11509_" + indexnum + "span").text(bz);
    }

    //发生人-部门（jbrbm）字段假如发生变化，明细表1费用承担部门（fyhsbm）值等于发生人-部门，赋值后，申请人可修改；
    jQuery("#field11469").bind("propertychange", function () {
        //setbm();
    ...
    function checkxtzmr() {
        ...
        for (var x = 0; x < si; x++) {
            ...
            //协调费没有大于1万元，写了“协但是填调费大于人民币1万元的支付见证人（需两人及以上）”，则不允许提交，提示“协调费金额没有大于1万元无需填写证明人，请删除！
            ...
            //新增部分
            if (kmval == "114" && xtzmr != "" && (ywje * _hv * _mhv < 10000)) {
                alert("协调费金额没有大于1万元无需填写证明人，请删除！ \n The coordination fee is not more than 10,000 yuan without filling out the certificate, please delete!");
                return false;
            }
